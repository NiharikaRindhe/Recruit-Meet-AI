...
# ✅ Correct CORS usage — this is the ONLY place we touch CORSMiddleware
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are an interview copilot helping a recruiter. "
                "Be neutral, supportive and never rude. "
                "Always return STRICT JSON with no extra commentary."
            ),
        },
        {"role": "user", "content": prompt},
    ]
...
async def start_session(payload: Dict[str, str]):
    ...
    # Do NOT touch transcript here; append happens via STT.
    # Upsert keeps a single row per meeting_id and lets us append transcript across sessions.
    supabase().table("meetings").upsert(row, on_conflict="meeting_id").execute()
    return {"session_id": meeting_id}
                ...
                for result in resp.results:
                    ...
                    if SUPABASE_ENABLED and session_id and is_final and text.strip():
                        try:
                            ...
                            print(
                                f"[stt:{source}] transcript append error: {e}"
                            )
...
def _extract_candidate_answer(
    question_text: str, turns: List[dict], interviewer_name: str = "", candidate_name: str = ""
) -> str:
    if not turns:
        return ""
    qn = _norm(question_text)
...
analytics: object with:
  - skills_match: integer 0–10
      How well the candidate's REAL skills (from RESUME and what they actually said in the interview) match the JD.
  ...
  - communication_experience: integer 0–10
      How clearly the candidate communicates and how well they explain their past experience and examples.
...
strengths:
  A short, neutral 2–3 line paragraph describing the key strengths shown in the RESUME and INTERVIEW.
  Focus on real evidence from their background and what they actually said.

weaknesses:
  A short, constructive 2–3 line paragraph about growth areas.
  Never be rude or harsh. If information is limited, say that evaluation is limited instead of criticizing.

interview_summary:
  A short, neutral 2–3 line paragraph summarizing how the interview went:
...
""".strip()
    ...
    # persist to meetings.ai_report (jsonb)
    try:
        _sb_upsert_ai_report(session_id, coerced)
    except Exception as e:
        print(f"[ai_summary] store report error: {e}")
...
First, infer the ideal expected_answer from JD/Resume, but do not assume skills that contradict the resume.
Then compare candidate_answer to expected_answer and fill verdict/score/explanation in a neutral and non-rude tone.
""".strip()
    data = _ollama_chat_json(prompt)
    ...
    return {
        ...
        "score": score,
        "score_pct": score_pct,
        "explanation": _to_bullets(
            explanation_raw, max_items=3, max_len=160
        ),
    }
