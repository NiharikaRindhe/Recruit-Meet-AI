...
# # if the "turns" table was deleted, auto-disable writes there
# _TURNS_ENABLED: Optional[bool] = None  # None=unknown, False=disabled, True=ok
...
# # ─────────────────────────────────────────────────────────────
# # Google STT helpers (mic/tab logic unchanged)
# # ─────────────────────────────────────────────────────────────
...
# # ─────────────────────────────────────────────────────────────
# # Session (kept) — now does NOT touch transcript (so it appends across sessions)
# # ─────────────────────────────────────────────────────────────
...
#     # IMPORTANT: do NOT include 'transcript' here; we keep existing transcript to allow appending across sessions
...
#     turns = _sb_get_turns(session_id, limit=500)
#     if turns:
#         return {"session_id": session_id, "turns": turns}
#     pseudo = _parse_transcript_to_turns(
#         _sb_get_meeting(session_id).get("transcript") or "",
#         _sb_get_meeting(session_id).get("interviewer_name") or "",
#         _sb_get_meeting(session_id).get("candidate_name") or "",
#     )
#     return {"session_id": session_id, "turns": pseudo}

# # ─────────────────────────────────────────────────────────────
# # WebSocket STT (unchanged logic + clean disconnect handling)
# # ─────────────────────────────────────────────────────────────
...
#                         if _TURNS_ENABLED is not False:
#                             try:
...
#                                 _TURNS_ENABLED = True
#                             except Exception as e:
#                                 if "PGRST205" in f"{e}" or "Could not find the table 'public.turns'" in f"{e}":
#                                     _TURNS_ENABLED = False
#                                 else:
#                                     print(f"[stt:{source}] db insert error: {e}")

#                         # always append to meetings.transcript
#                         try:
...
# def _sb_get_turns(session_id: str, limit: int = 200) -> List[dict]:
#     if not SUPABASE_ENABLED:
#         return []
#     global _TURNS_ENABLED
#     if _TURNS_ENABLED is False:
#         return []
#     try:
#         q = (
#             supabase()
...
#             _TURNS_ENABLED = False
#             return []
#         print(f"[sb] get turns error: {e}")
#         return []

# # NEW: store AI summary JSON in ai_report table
# def _sb_upsert_ai_report(session_id: str, report: dict) -> None:
#     if not SUPABASE_ENABLED:
#         return
#     try:
#         supabase().table("ai_report").upsert(
#             {"meeting_id": session_id, "report": report},
#             on_conflict="meeting_id"
#         ).execute()
#     except Exception as e:
#         print(f"[sb] upsert ai_report error: {e}")

# # transcript → pseudo turns (fallback when turns table is gone)
...
# # ─────────────────────────────────────────────────────────────
# # Candidate answer extraction (name-aware + transcript fallback)
# # ─────────────────────────────────────────────────────────────
# def _norm(s: str) -> str:
#     s = s.lower().strip()
...
#     turns = _sb_get_turns(session_id, limit=limit)
#     if turns:
#         return turns
#     mtg = _sb_get_meeting(session_id)
...
# # ─────────────────────────────────────────────────────────────
# # Summary payload coercer — ALWAYS return your requested JSON shape
# # ─────────────────────────────────────────────────────────────
...
# # ─────────────────────────────────────────────────────────────
# # AI endpoints (logic same; outputs short bullet lists where applicable)
# # ─────────────────────────────────────────────────────────────
# @app.post("/ai/turn")
# async def ai_turn(payload: Dict[str, str] = Body(...)):
#     session_id = (payload or {}).get("session_id", "")
#     turns = _recent_turns(session_id, 200)
#     if not turns:
...
#     prompt = f"""
# Return STRICT JSON with keys: next_question, expected_answer, validation.
# Use this info:
...
# CURRENT TURN:
# Speaker: {(current_turn.get("speaker_name") or current_turn.get("speaker") or current_turn.get("source"))}
# Text: {current_turn.get("text","")}
# """.strip()
#     data = _ollama_chat_json(prompt)
#     exp_raw = (data.get("expected_answer") or data.get("raw") or "").strip()
#     val_raw = (data.get("validation") or "").strip()
#     return {
#         "session_id": session_id,
#         "current_turn_id": current_turn.get("id"),
#         "next_question": (data.get("next_question") or "").strip(),
#         "expected_answer": _to_bullets(exp_raw, max_items=6),
#         "validation": _to_bullets(val_raw, max_items=3, max_len=160),
#     }
...
#     # NEW: persist to ai_report as JSON
#     try:
...
# @app.post("/ai/suggest")
# async def ai_suggest(payload: Dict[str, Any] = Body(...)):
#     session_id = (payload or {}).get("session_id")
...
#         "turns_enabled": _TURNS_ENABLED,
#     }

# if __name__ == "__main__":
#     uvicorn.run("server_supabase:app", host="127.0.0.1", port=8000, reload=True)
...
# UPDATED: store AI summary into meetings.ai_report (jsonb column),
# and fall back to a separate ai_report table if present.
# def _sb_upsert_ai_report(session_id: str, report: dict) -> None:
#     if not SUPABASE_ENABLED:
#         return
#     # Ensure JSON-serializable (clean any non-serializable objects)
#     try:
...
#     # Primary path: update jsonb column on meetings
#     try:
#         supabase().table("meetings").update({"ai_report": safe_report}).eq("meeting_id", session_id).execute()
#         return
#     except Exception as e:
#         print(f"[sb] update meetings.ai_report error: {e} — trying ai_report table fallback")

#     # Fallback: if you actually have a separate ai_report table with (meeting_id, report)
#     try:
#         supabase().table("ai_report").upsert(
#             {"meeting_id": session_id, "report": safe_report},
#             on_conflict="meeting_id"
#         ).execute()
#     except Exception as e2:
#         print(f"[sb] upsert ai_report table error: {e2}")
def _sb_upsert_ai_report(session_id: str, report: dict) -> None:
    if not SUPABASE_ENABLED:
        return
    try:
    ...
    try:
        # Write ONLY to the meetings.ai_report (jsonb) column for this meeting_id
        supabase().table("meetings").update({"ai_report": safe_report}).eq("meeting_id", session_id).execute()
    except Exception as e:
        # Log the error; do not write anywhere else
...
""".strip()
    ...
    # persist to meetings.ai_report (jsonb), with table fallback
    try:
        _sb_upsert_ai_report(session_id, coerced)
    except Exception as e:
        print(f"[ai_summary] store report error: {e}")
